...
{
    private DialogueSequenceSO _sequenceSO;
    ...
    private SerializedProperty _goodOutcomeSequenceProp;
    private SerializedProperty _neutralOutcomeSequenceProp;
    private SerializedProperty _badOutcomeSequenceProp;
    ...
    private Dictionary<string, List<string>> _incomingReferencesCache; // Cache for performance
    private bool _showIncomingReferences = false;

    private void OnEnable()
    {
        _sequenceSO = (DialogueSequenceSO)target;
        ...
        _goodOutcomeSequenceProp = serializedObject.FindProperty("goodOutcomeSequence");
        _neutralOutcomeSequenceProp = serializedObject.FindProperty("neutralOutcomeSequence");
        _badOutcomeSequenceProp = serializedObject.FindProperty("badOutcomeSequence");
        ...
        // Initialize incoming references cache
        _incomingReferencesCache = null; // Will be built on-demand
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        SequenceType currentType = (SequenceType)_typeProp.enumValueIndex;

        if (currentType == SequenceType.Linear)
        {
        ...
        else if (currentType == SequenceType.RandomOutcome)
        {
            EditorGUILayout.PropertyField(_goodOutcomeSequenceProp);
            EditorGUILayout.PropertyField(_neutralOutcomeSequenceProp);
            EditorGUILayout.PropertyField(_badOutcomeSequenceProp);
        }
        ...
        else
        {
            ...
            for (int i = 0; i < _nodeListProp.arraySize; i++)
            {
                ...
                // Search filter - skip nodes that don't match (performance optimization)
                ...
                if (nodeIDVal == _startingNodeIDProp.stringValue && currentType == SequenceType.Linear)
                {
                ...
                if (GUILayout.Button(new GUIContent($"<color={nodeLabelColor}><b>{nodePrefix}{displayNodeID}{(IsNodeReferencedByChoice(nodeIDVal, _sequenceSO) && !(nodeIDVal == _startingNodeIDProp.stringValue && currentType == SequenceType.Linear) ? " (Targeted)" : "")}</b></color>"), _nodeIDLabelStyle, GUILayout.ExpandWidth(false)))
                {
                ...
                if (_nodeFoldouts[uniqueFoldoutKey])
                {
                    ...
                    for (int j = 0; j < linesProp.arraySize; j++)
                    {
                        ...
                        SerializedProperty animationProp = lineElementProp.FindPropertyRelative("animationToPlay");
                        SerializedProperty loopAnimProp = lineElementProp.FindPropertyRelative("loopAnimation");
                        SerializedProperty animFloatProp = lineElementProp.FindPropertyRelative("animationFloatValue");
                        ...
                        AnimationType animType = (AnimationType)animationProp.enumValueIndex;
                        if (animType != AnimationType.None)
                        {
                            animationInfo = $"   🎭 {animType} (Loop)"; // All animations loop by default via Trigger + Bool pattern
                            if (animFloatProp.floatValue > 0f)
                                animationInfo += $" | Value: {animFloatProp.floatValue}";
                        }
                        else
                        {
                            animationInfo = "   🎭 No Animation";
                        }

                        EditorGUILayout.LabelField(animationInfo, _richMiniLabelStyle);
                    ...
                    for (int k = 0; k < choicesProp.arraySize; k++)
                    {
                        ...
                        {
                            DialogueSequenceSO targetSeq = (DialogueSequenceSO)nextSeqSO_choiceProp.objectReferenceValue;
    ...
    private void FocusOnNode(string targetNodeID, int originatingNodeIndex)
    {
        ...
        if (!foundAndOpened)
        {
            Debug.LogWarning($"[DialogueSequenceSOEditor] Node ID '{targetNodeID}' not found in the current sequence ('{_sequenceSO.name}') to focus/expand.");
        }
    }

    private bool IsNodeReferencedByChoice(string nodeIDToCheck, DialogueSequenceSO currentSequence)
    {
        if (string.IsNullOrEmpty(nodeIDToCheck) || currentSequence == null || currentSequence.nodeList == null) return false;

        foreach (DialogueNodeData sourceNode in currentSequence.nodeList)
        {
            if (sourceNode == null || sourceNode.choices == null) continue;

            foreach (DialogueChoiceData choice in sourceNode.choices)
            {
    ...
    private bool NodeContainsSearchText(SerializedProperty nodeElementProp, string searchText)
    {
        ...
        SerializedProperty linesProp = nodeElementProp.FindPropertyRelative("lines");
        int linesToCheck = Mathf.Min(linesProp.arraySize, 5); // Limit to first 5 lines for performance
        ...
        SerializedProperty choicesProp = nodeElementProp.FindPropertyRelative("choices");
        int choicesToCheck = Mathf.Min(choicesProp.arraySize, 5); // Limit to first 5 choices for performance
    ...
    private void BuildIncomingReferencesCache()
    {
        ...
        // Build cache of which nodes are referenced by which other nodes
        for (int i = 0; i < _sequenceSO.nodeList.Count; i++)
        {
            ...
            foreach (var choice in sourceNode.choices)
            {
                ...
                if (choice.nextSequenceSO != null) continue; // Skip cross-sequence references
